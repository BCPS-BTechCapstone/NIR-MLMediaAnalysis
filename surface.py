...
def plot_data(data, output_filename, elev, azim, show_plot, method=None):
    ...
    Y = np.array([data[col].values for col in data.columns[1:]])

    # Create a meshgrid for X and Z
    X, Z = np.meshgrid(X, Z)

    # Ensure Y has the same shape as X and Z
    if Y.shape != X.shape:
        Y = Y.reshape(Z.shape)

    # Plot the data in 3D as a surface plot
    ...
    surf = ax.plot_surface(X, Z, Y, cmap=cmap, alpha=0.7)
    ...
    fig.colorbar(surf, ax=ax, fraction=0.02, pad=0, label='Time (min)')
